...
export function formatNonStandardDate (date: string, currentFormat = "YYYY-MM-DD hh:mm:ss A", format = "YYYY-MM-DD HH:mm:ss") {
...
export function diffTimes (startTime: string, endTime: string, unit = "hours", inputFormat = "YYYY-MM-DD HH:mm:ss") {
	const startMoment = moment(startTime, inputFormat)
	const endMoment = moment(endTime, inputFormat)
...
export function getAllDatesInRange (
	...
	unitOfRange: unitOfTime.Diff = "months",
	resolution: unitOfTime.Diff = "days"
) {
	const includeEndDateInRange = 1
	const calculatedStartDate = moment(startDate) || moment().subtract(howFarBack, unitOfRange)
	const calculatedEndDate = moment(endDate).add(includeEndDateInRange, resolution) || moment().add(howFarBack, unitOfRange)

	const diff = calculatedEndDate.diff(calculatedStartDate, resolution)

	if (!calculatedStartDate.isValid() || !calculatedEndDate.isValid() || diff <= 0) {
		return []
	}

	return Array.from({ length: diff }, () => calculatedEndDate.subtract(1, resolution).format(dateFormat))
}
...
export function getDatesForWeek (someDateOfWeek = moment().format("YYYY-MM-DD"), format = "YYYY-MM-DD") {
	...
	return [startDateOfWeek.format(format), ...Array.from({ length: diff - 1 }, () => startDateOfWeek.add(1, resolution).format(format))]
}
